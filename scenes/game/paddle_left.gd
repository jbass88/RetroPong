...
func _process(delta):
	...
	if GlobalData.PaddleControlLeft == GlobalData.PaddleControlType.PLAYER_1:
		...
		if Input.is_action_pressed("paddle_right_down"):
			move += 1
		position.y += move * player_speed * delta
	elif GlobalData.PaddleControlLeft == GlobalData.PaddleControlType.AI_EASY:
	...
	elif GlobalData.PaddleControlLeft == GlobalData.PaddleControlType.AI_MEDIUM:
	...
	elif GlobalData.PaddleControlLeft == GlobalData.PaddleControlType.AI_HARD:
